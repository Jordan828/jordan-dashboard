...
def mini_spark(pct, color, w=80, h=32):
    ...
    gid = abs(int(pct*100))
    return f'<svg width="{w}" height="{h}" viewBox="0 0 {w} {h}" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="g{gid}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="{color}" stop-opacity="0.3"/><stop offset="100%" stop-color="{color}" stop-opacity="0.02"/></linearGradient></defs><path d="{fill_path}" fill="url(#g{gid})"/><path d="{path}" fill="none" stroke="{color}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>'
...
def generate_html(quotes, news_items, indices, now):
    ...
    for sym, name in idx_map.items():
        ...
        arrow = "▲" if pct >= 0 else "▼"
        idx_html += f'<div class="idx-card"><div class="idx-name">{name}</div><div class="idx-price">{price:,.2f}</div><div class="idx-chg" style="color:{color}">{arrow} {abs(pct):.2f}%</div></div>'
    ...
    for sym in TICKERS:
        ...
        cname   = COMPANY_NAMES.get(sym, sym)
        spark   = mini_spark(pct, color)
        up_color = "#00c98d" if upside.startswith("+") else "#ff4d6a"
        watch_cards += f'<div class="wcard"><div class="wcard-top"><div><div class="wcard-sym">{sym}</div><div class="wcard-name">{cname}</div><div class="wcard-sector">{sector}</div></div><div class="wcard-spark">{spark}</div></div><div class="wcard-price">${price:,.2f}</div><div class="wcard-chg" style="color:{color}">{arrow} {abs(pct):.2f}% (${abs(chg):.2f})</div><div class="wcard-meta"><span class="wcard-tag" style="color:{rat_col};border-color:{rat_col}20;background:{rat_col}12">{rating}</span><span class="wcard-tag" style="color:{act_color};border-color:{act_color}20;background:{act_color}12" title="{reason}">{action}</span><span class="wcard-tag" style="color:{up_color}">{upside} upside</span></div></div>'
    ...
    for n in news_items[:12]:
        ...
        news_html += f'<div class="news-row"><div class="news-left"><span class="news-sym">{sym_n}</span><span class="news-sent" style="color:{sent_color}">{sent_arrow} {sent}</span></div><div class="news-title"><a href="{url_n}" target="_blank">{title_n}</a></div><div class="news-date">{pub}</div></div>'
    ...
    for sym in TICKERS:
        q     = quotes.get(sym, {})
        sp    = q.get('price', 0)
        spct  = q.get('changesPercentage', 0)
        sc    = '#00c98d' if spct >= 0 else '#ff4d6a'
        sarr  = '▲' if spct >= 0 else '▼'
        sname = COMPANY_NAMES.get(sym, '')[:20]
        sidebar_items += f'<div class="sb-item"><div class="sb-left"><span class="sb-sym">{sym}</span><span class="sb-name">{sname}</span></div><div class="sb-right"><span class="sb-price" style="color:{sc}">${sp:,.2f}</span><span class="sb-pct" style="color:{sc}">{sarr} {abs(spct):.2f}%</span></div></div>'

    sl = json.dumps(sector_labels)
    sc2 = json.dumps(sector_counts)
    as2 = json.dumps(a_syms)
    ab  = json.dumps(a_buys)
    ah  = json.dumps(a_holds)
    ase = json.dumps(a_sells)
...
:root{{--bg:#0f1117;--sidebar:#13151e;--card:#1a1d27;--card2:#1f2233;--border:#252a3a;--text:#e8eaf0;--sec:#8b90a7;--muted:#4a5068;--green:#00c98d;--red:#ff4d6a;--amber:#f5a623;--blue:#4d9fff;--r:10px;--gap:14px}}
...
.idx-bar{{display:flex;overflow-x:auto;background:var(--sidebar);border-bottom:1px solid var(--border);padding:0 24px}}
.idx-card{{padding:10px 24px 10px 0;margin-right:24px;border-right:1px solid var(--border);flex-shrink:0}}
...
.wcard-sym{{font-size:15px;font-weight:700}}
...
@media(max-width:900px){{.layout{{grid-template-columns:1fr}}.sidebar{{display:none}}.kpi-row,.charts-row{{grid-template-columns:repeat(2,1fr)}}.wgrid{{grid-template-columns:repeat(2,1fr)}}.news-row{{grid-template-columns:1fr}}}}
</style>
</head>
<body>
<div class="topbar">
...
<div class="layout">
  <div class="sidebar"><div class="sb-section"><div class="sb-title">Watchlist</div>{sidebar_items}</div></div>
  <div class="main">
    ...
    <div class="news-card"><div class="news-hdr">📰 Latest News</div>{news_html}</div>
  </div>
</div>
<div class="footer">🤖 Auto-generated by Jordan's AI Financial Advisor · Data via FMP · Updates every weekday 9:00 AM MYT<br>⚠️ Not financial advice.</div>
<script>
const sColors=['#a78bfa','#4d9fff','#7f77dd','#00c98d','#f5a623','#ff4d6a'];
new Chart(document.getElementById('donut'),{{type:'doughnut',data:{{labels:{sl},datasets:[{{data:{sc2},backgroundColor:sColors,borderColor:'#0f1117',borderWidth:3,hoverOffset:6}}]}},options:{{responsive:true,maintainAspectRatio:false,cutout:'65%',plugins:{{legend:{{position:'right',labels:{{color:'#8b90a7',font:{{size:10}},boxWidth:10,padding:8}}}}}}}}}}});
new Chart(document.getElementById('analyst'),{{type:'bar',data:{{labels:{as2},datasets:[{{label:'Buy',data:{ab},backgroundColor:'rgba(0,201,141,.8)',borderRadius:3}},{{label:'Hold',data:{ah},backgroundColor:'rgba(245,166,35,.8)',borderRadius:3}},{{label:'Sell',data:{ase},backgroundColor:'rgba(255,77,106,.8)',borderRadius:3}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{labels:{{color:'#8b90a7',font:{{size:10}},boxWidth:10}}}}}},scales:{{x:{{stacked:true,grid:{{display:false}},ticks:{{color:'#8b90a7',font:{{size:10}}}}}},y:{{stacked:true,grid:{{color:'rgba(37,42,58,.8)'}},ticks:{{color:'#8b90a7',font:{{size:10}}}}}}}}}}}}});
async function triggerRefresh(){{
  ...
  try{{
    ...
    if(r.status===204){{btn.innerHTML='⏳ Wait 30s then reload';setTimeout(()=>{{btn.innerHTML='✅ Reload now!';btn.disabled=false;}},35000);}}
